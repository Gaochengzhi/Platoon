...
def communicate(plexe, topology):
    ...
    for vid, l in topology.items():
        ...
        try:
            # vidtype = traci.vehicle.getTypeID(vid)
            # headway = traci.vehicletype.getTau(vidtype)
            speed = traci.vehicle.getSpeed(vid)
            plexe.set_path_cacc_parameters(vid,distance=(speed*0.3))
        ...
        if "leader" in l.keys():
            ...
            # vidtypein = traci.vehicle.getTypeID(vid)
            try:
                # vidtype = traci.vehicle.getTypeID(vid)
                # headway = traci.vehicletype.getTau(vidtype)
                max_speed_type = traci.vehicle.getMaxSpeed(vid)
                allow_speed = min(traci.vehicle.getAllowedSpeed(vid),max_speed_type)
                plexe.set_cc_desired_speed(vid,allow_speed)
            except Exception as e:
                print(e)
                pass
        if "front" in l.keys():
