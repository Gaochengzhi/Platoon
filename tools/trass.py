...
def run_in_process(file_path):
    ...
    for name, group in grouped:
        ...
        if all(group['idv'].astype(str).str.startswith("p.")):
            ...
            color = 'green'
            ls = 'solid'
        plt.plot(group[xl], group[yl], linestyle=ls, label=name, linewidth=0.5, color=color)
    # plt.legend()
    plt.gca().xaxis.set_major_formatter(plt.FuncFormatter(lambda x, pos: '{:.0f}'.format(x/100)))
